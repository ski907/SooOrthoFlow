...
def compute_mosaic_bounds(ortho_dir):
    ...
    with Pool(cpu_count()) as pool:
        results = pool.map(_compute_single_image_bounds, tif_files)
    
    # Filter out None results and extract bounds
    valid_results = [r for r in results if r is not None]
    
    if not valid_results:
        raise ValueError("No valid images with world files found")
    
    x_mins = [r[0] for r in valid_results]
    x_maxs = [r[1] for r in valid_results]
    y_mins = [r[2] for r in valid_results]
    y_maxs = [r[3] for r in valid_results]
    
    return min(x_mins), max(x_maxs), min(y_mins), max(y_maxs)


def _compute_single_image_bounds(tif_path):
    """Helper function to compute bounds for a single image (for parallel processing)"""
...
def _load_and_prepare_image(args):
    """
    Helper function to load and prepare a single image (for parallel processing)
    Returns all the data needed for merging into mosaic
    """
    tif_path, tfw_path, x_min, y_max, resolution, mosaic_width, mosaic_height = args
...
def create_seam_carved_mosaic(ortho_dir, output_path, resolution=None, 
                              seam_method='gradient', save_seam_map=False, n_jobs=None):
    """
    VECTORIZED + PARALLEL: Create mosaic using seam carving - finds optimal non-blended boundaries
    
    Strategy:
    1. Load and prepare all images in PARALLEL
    2. Process images left-to-right (sorted by X position)
    3. For each new image, find optimal seam in overlap region
    4. Use hard cut at seam (no blending)
    ...
    # Compute overall bounds (parallelized)
    ...
    # PARALLEL: Load and prepare all images at once
    print(f"\nLoading {len(image_data)} images in PARALLEL...")
    
    load_args = [
        (tif_path, tfw_path, x_min, y_max, resolution, mosaic_width, mosaic_height)
        for _, tif_path, tfw_path in image_data
    ]
    
    with Pool(n_jobs) as pool:
        prepared_images = pool.map(_load_and_prepare_image, load_args)
    
    # Filter out any failed loads
    prepared_images = [img for img in prepared_images if img is not None]
    
    print(f"Successfully loaded {len(prepared_images)} images")
    print(f"\nProcessing images left-to-right with seam carving (VECTORIZED)...")
    
    # Sequential merging with seam carving
    for i, img_data in enumerate(prepared_images, 1):
        print(f"\n[{i}/{len(prepared_images)}] Processing {img_data['tif_path'].name}")
        
        img_region = img_data['img_region']
        valid_mask = img_data['valid_mask']
        mosaic_col_start = img_data['mosaic_col_start']
        mosaic_row_start = img_data['mosaic_row_start']
        mosaic_col_end = img_data['mosaic_col_end']
        mosaic_row_end = img_data['mosaic_row_end']
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Create mosaic from orthorectified images - VECTORIZED + PARALLEL VERSION'
    )
